...
#fullArmBandDataRows = getRowsWithFullData(TEST_DATA)
TESTnonNaArmBandData <- removeColumnsThatAreMostlyNA(TEST_DATA,na_ratio_threshold=0.0)
TESTvariantArmBandData <- filterOutNonVariantColumns(TESTnonNaArmBandData)
#variantArmBandData <- convertUsernameToBinaryVariables(variantArmBandData)
TESTrelevantArmBandData <- filterOutIrrelevantVariantColumns(TESTvariantArmBandData)

problem_id_index <- getColumnIndexByLabel("problem_id", TESTrelevantArmBandData)
TEST_TESTING_SET <- predict(preProcessedTrainingSet, TESTrelevantArmBandData[,-problem_id_index])
TEST_PREDICTIONS <- predict(armBandPredictionModel, newdata=TEST_TESTING_SET)
armBandPredictionResults <- confusionMatrix(testingSet$classe, TEST_PREDICTIONS)
